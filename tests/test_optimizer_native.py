...
from aegisswarm.optimizer_native_proof import run_native_objective_confirmation
...
def test_native_evaluator_exposes_full_metric_schema():
    metrics = evaluate_native_objective(DEFAULT_NATIVE_OBJECTIVE, EvalConfig(seeds=(0, 1)))
    for key in (
        "fitness",
        "loss",
        "asset_survival_rate",
...
def test_native_oracle_deduplicates_canonical_vectors():
    with NativeBudgetedOracle(EvalConfig(seeds=(0,)), budget=4, workers=1) as oracle:
...
def test_native_seed_blocks_are_fresh():
    ...
    assert len(dev) == 400 and NATIVE_OBJECTIVE_DEV_SEEDS[0] == 9000 and NATIVE_OBJECTIVE_DEV_SEEDS[-1] == 9399
    assert len(confirm) == 400 and NATIVE_OBJECTIVE_CONFIRM_SEEDS[0] == 10000 and NATIVE_OBJECTIVE_CONFIRM_SEEDS[-1] == 10399
...
def test_native_confirmation_requires_explicit_freeze(tmp_path):
    ...
    with pytest.raises(RuntimeError, match="not frozen"):
        run_native_objective_confirmation(source_dir=source, out_dir=tmp_path / "confirm", workers=1)
