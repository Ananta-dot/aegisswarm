...
from .splits import NATIVE_OBJECTIVE_CONFIRM_SEEDS, NATIVE_OBJECTIVE_DEV_SEEDS, TRAIN_SEEDS


PROTOCOL_ID = "aegisswarm-optimizer-native-objective-v1"
...
def _evaluate_native_on_seeds(vector, seeds, scenario_kwargs):
    ...
    for seed in seeds:
        ...
        metrics = Simulator.evaluate_policy(scenario, OptimizerNativePolicy(vector))
...
def compare_rule_vs_native(rule_eval, native_eval):
    ...
    for i, metric in enumerate(METRICS):
        ...
        comparison[metric] = {
            ...
            "probability_native_better": float(stats["probability_axplorer_better"]),
...
def _write_report(path, mode, seeds, fixed, rule, native, comparison):
    ...
    lines = [
        "# AegisSwarm Optimizer-Native Objective Report",
        "",
        ...
        f"| native_objective | {100*native['asset_survival_rate']['mean']:.2f}% | [{100*native['asset_survival_rate']['ci95'][0]:.2f}%, {100*native['asset_survival_rate']['ci95'][1]:.2f}%] |",
        "",
        "## Primary representation ablation",
        "",
        f"- Native minus 60-token rule objective: **{100*s['native_minus_rule']:+.2f} pp**",
    ...
    if mode == "development":
        ...
        lines += ["Quick integration check only; do not interpret as evidence."]
    else:
...
def _evaluate(out_dir, mode, seeds, rule_runs, native_runs, workers):
    rule_programs = [np.asarray(run["best_program"], dtype=np.int16) for run in rule_runs]
    native_vectors = [np.asarray(run["best_objective"], dtype=float) for run in native_runs]

    fixed_eval = evaluate_optimizer_only(seeds, SCENARIO_KWARGS)
    rule_eval = evaluate_hybrid_program_runs(rule_programs, seeds, SCENARIO_KWARGS, workers=workers)
    native_eval = evaluate_native_runs(native_vectors, seeds, SCENARIO_KWARGS, workers=workers)
    ...
    result = {
        ...
        "native_objective": native_summary,
        "paired_representation_comparison": comparison,
    }
    _save_json(Path(out_dir) / "results.json", result)
    _write_report(Path(out_dir) / "REPORT.md", mode, seeds, fixed_summary, rule_summary, native_summary, comparison)

    s = comparison["asset_survival_rate"]
    print("\n=== OPTIMIZER-NATIVE OBJECTIVE ===", flush=True)
    print(f"fixed_optimizer survival: {fixed_summary['asset_survival_rate']['mean']:.3f}", flush=True)
    print(
    ...
    print(
        f"native_objective survival:{native_summary['asset_survival_rate']['mean']: .3f} "
...
def run_native_objective_development(
    out_dir="artifacts/optimizer_native_dev",
    workers=4,
    quick=False,
    force_train=False,
):
    ...
    protocol = {
        ...
        "workers": int(workers),
        "representation_ablation": ["60-token rule objective", "10-parameter optimizer-native objective"],
    }
    _save_json(out_dir / "protocol.json", protocol)

    print(
        f"AegisSwarm optimizer-native {'quick' if quick else 'development'} | "
    ...
    for i, search_seed in enumerate(search_seeds, start=1):
        print(f"\n=== paired representation run {i}/{len(search_seeds)} seed={search_seed} ===", flush=True)
        rule_runs.append(
            _load_or_train(
        ...
        native_runs.append(
            _load_or_train(
                runs_dir / f"native_objective_seed_{search_seed}.json",
                "optimizer_native_local",
                search_seed,
                train_native_local,
                force=force_train,
                config=config,
...
def run_native_objective_confirmation(
    source_dir="artifacts/optimizer_native_dev",
    out_dir="artifacts/optimizer_native_confirm",
    workers=4,
):
    ...
    if not bool(protocol.get("architecture_frozen", False)):
        raise RuntimeError(
            "Optimizer-native architecture is not frozen. Do not consume confirmation seeds. "
    ...
    for seed in SEARCH_SEEDS:
        ...
        npth = runs_dir / f"native_objective_seed_{seed}.json"
        if not rp.exists() or not npth.exists():
            raise FileNotFoundError(f"Missing frozen run artifacts for search seed {seed}")
